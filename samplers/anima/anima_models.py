...
# Vendored for inference only: the upstream trainer imports block-swap offloading
# and IPEX helpers here. Sample generation never enables block swap (the DiT is
# small enough to stay resident on a single GPU), so we provide light stubs
# instead of pulling in the trainer's device/offloading machinery.
class _NoOffloadStub:
    """Placeholder for library.custom_offloading_utils.

    Only referenced as a type annotation on ``MiniTrainDIT.offloader`` and inside
    ``enable_block_swap`` (never called during sampling), so a bare class with a
    ``ModelOffloader`` attribute is enough to keep the module importable.
    """
    class ModelOffloader:  # pragma: no cover - never instantiated for sampling
        pass


custom_offloading_utils = _NoOffloadStub
